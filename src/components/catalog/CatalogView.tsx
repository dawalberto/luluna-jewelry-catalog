...
export default function CatalogView() {
    ...
    if (selectedCategories.length > 0 || selectedSubcategoryKeys.length > 0) {
      filtered = filtered.filter((product: any) => {
        const productCategories: string[] = Array.isArray(product?.categories)
          ? product.categories
          : []
        const productSubKeys: string[] = Array.isArray(product?.subcategoryKeys)
  ...
  const sortedProducts = useMemo(() => {
    ...
    tagAndCollectionFilteredProducts,
    sortBy,
    pricingConfig,
  ...
  const collectionGroups = useMemo(() => {
    if (sortBy !== "collections")
      return [] as Array<{
        id: string
        label: string
        products: typeof tagAndCollectionFilteredProducts
      }>
    ...
    <div className="container mx-auto px-3 py-4 md:px-4 md:py-10">
          ...
          >
            <defs>
              ...
              <linearGradient id="watercolorGradient" x1="0%" y1="0%" x2="100%" y2="0%">
                <stop
                  offset="0%"
                  style={{ stopColor: "var(--color-primary)", stopOpacity: 0.65 }}
                />
                <stop
                  offset="20%"
                  style={{ stopColor: "var(--color-primary)", stopOpacity: 0.85 }}
                />
                <stop
                  offset="50%"
                  style={{ stopColor: "var(--color-primary)", stopOpacity: 0.9 }}
                />
                <stop
                  offset="80%"
                  style={{ stopColor: "var(--color-primary)", stopOpacity: 0.85 }}
        ...
        <div className="flex items-center justify-between gap-4 border-b border-(--color-border) pb-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <span className="text-[10px] md:text-xs uppercase tracking-[0.15em] text-(--color-muted) hidden md:inline">
                {t.catalog.order}
              </span>
              <select
                value={sortBy}
              ...
              >
                <option value="collections">
                  {(t.catalog as any).sortCollections || "Colecciones"}
                </option>
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center justify-between p-6 border-b border-(--color-border)">
            <h2 className="text-lg font-heading font-medium text-(--color-text) tracking-wide">
              {t.catalog.filters}
            </h2>
            <button
              onClick={() => setShowFilters(false)}
                        ...
                        return (
                          <div key={cat.id} className="flex flex-col gap-2">
                            <button
                              ...
                              className="group flex w-full items-center justify-between gap-3 text-sm text-left py-2 px-3 rounded-lg transition-all text-(--color-text) hover:bg-black/5"
                            >
                              <span className="truncate">{catLabel}</span>
                              <svg
                                className={`h-4 w-4 shrink-0 text-(--color-muted) transition-transform duration-200 group-hover:text-(--color-text) ${
              ...
              <div>
                ...
                <div className="flex flex-col gap-2">
                  {dbCollections.map((collection) => {
                    ...
                    const label =
                      collection.title?.[locale] ?? collection.title?.es ?? collection.id
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          setSelectedCollection(isSelected ? undefined : collection.id)
                        }
                        className={`text-sm text-left py-2 px-3 rounded-lg transition-all ${
                          isSelected
                            ? "bg-(--color-primary)/10 text-(--color-primary) font-medium"
      ...
      {globalDiscount?.active && globalDiscount.percent > 0 && (
        <div className="mb-10 md:mb-16 bg-[#F4F1EA] px-4 py-8 md:px-8 md:py-12 text-center">
          <div className="text-xs font-medium text-(--color-primary) uppercase tracking-[0.25em] mb-3">
            {t.catalog.promoTitle}
          </div>
          <div className="text-2xl md:text-4xl font-heading font-medium text-(--color-text) mb-3 md:mb-5 tracking-tight">
            ...
            <span className="ml-3 text-(--color-primary) italic font-accent">
              {globalDiscount.percent}% OFF
            </span>
          </div>
          {(() => {
            ...
            return localizedDescription ? (
              <div className="text-sm md:text-base text-(--color-muted) max-w-2xl mx-auto font-light leading-relaxed">
                {localizedDescription}
              </div>
            ) : (
              <div className="text-sm md:text-base text-(--color-muted) max-w-2xl mx-auto font-light leading-relaxed">
                {t.catalog.promoDescription}
              </div>
            )
          })()}
        </div>
