...
export default function ProductCard({
  ...
  const cardSrcSetWidths = [400, 800] as const
  const cardSizes =
  ...
  const snapToNearest = () => {
    ...
    const nextIndex = Math.max(
      0,
      Math.min(images.length - 1, Math.round(scroller.scrollLeft / width))
    )
  ...
  const categoryLabel = useMemo(() => {
    ...
    const base = (
      typeof fromDb === "string" && fromDb.length > 0
        ? fromDb
        : (t as any)?.categories?.[first] ?? first
    ) as string
      ...
      >
        {hasMultipleImages ? (
          ...
          >
            {images.map((src, idx) => (
              <div key={src || idx} className="h-full w-full shrink-0 snap-start">
                <img
                  src={getCloudinaryUrl(src, { width: 800 })}
                  srcSet={getResponsiveSrcSet(src, [...cardSrcSetWidths])}
                  sizes={cardSizes}
          ...
          <img
            ...
            srcSet={getResponsiveSrcSet(imageUrl, [...cardSrcSetWidths])}
            sizes={cardSizes}
      ...
      <div className="pt-5 pb-2 text-center">
        ...
        <div className="mt-2 flex items-center justify-center gap-3">
          {finalPrice == null ? (
            ...
            <>
              <span className="text-xs text-(--color-muted) line-through decoration-1 font-light">
